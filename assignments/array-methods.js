...
runners.map(runner => {return allCaps.push(runner.first_name.toUpperCase())})
...
ticketPriceTotal.push(runners.reduce((a, b) => {return a + b["donation"]}, 0));
...
// Problem 1
// The director needs a list of the companies involved and the last name of the runner representing them so the tee shirts can be made for each runner with the company they're representing.
let shirtInfo = [];
runners.forEach(runner => {return shirtInfo.push(`${runner.company_name}: ${runner.last_name}`)});
console.log(shirtInfo);

// Problem 2
// There is a special thank you given to the companies who donated $200 or more, so the director needs a list of the emails of those who with higher donations.
let specialThanks = [];
runners.filter(runner => {
    if(runner.donation >= 200) {
        specialThanks.push(runner.email);
    }
});
console.log(specialThanks);

// Problem 3
// In order to give the event information, the director needs a list of just the email addresses.
const mailingList = runners.map(a => {return a.email});
